...
def upload_to_cloudinary(media_url: str, media_type: str, media_number: int, quality: str) -> str:
    """
    Upload media to Cloudinary and return the high quality hosted URL.
    ...
    try:
        ...
        logger.info(f"Uploading {media_type} {media_number} ({quality}) to Cloudinary with high quality settings...")
        
        # Set resource type based on media type
        resource_type = "video" if media_type == "video" else "image"
        
        # Configure upload parameters for high quality
        upload_params = {
            "resource_type": resource_type,
            "public_id": f"instagram_media_{media_number}_{quality}",
            "folder": "instagram_downloads",
            "overwrite": True,
            "quality": "auto:best",  # Automatically optimize for best quality
        ...
        if media_type == "video":
            upload_params.update({
                "video_codec": "h264",
                "audio_codec": "aac",
                "bit_rate": "2m",  # 2 Mbps for high quality
                "quality": "auto:best"
            })
        else:
            ...
            upload_params.update({
                "quality": "auto:best",
                "dpr": "2.0"  # High DPI for retina displays
            })
        
        # Upload to Cloudinary
        upload_result = cloudinary.uploader.upload(media_url, **upload_params)
        
        # Get high quality URL with transformations
        if media_type == "video":
            # For videos, apply quality optimization
            cloudinary_url = cloudinary.CloudinaryVideo(upload_result['public_id']).build_url(
                quality="auto:best",
                fetch_format="auto",
                video_codec="h264",
                audio_codec="aac"
            ...
            # For images, apply quality optimization and format conversion
            cloudinary_url = cloudinary.CloudinaryImage(upload_result['public_id']).build_url(
                quality="auto:best",
                fetch_format="auto",
                dpr="2.0",
                flags="progressive"
        ...
        logger.info(f"Successfully uploaded to Cloudinary with high quality: {cloudinary_url}")
...
async def root():
    ...
    return {
        ...
        "features": {
            ...
            "cloudinary_hosting": "Upload media to Cloudinary in high quality (only for URLs ending with &dl=1)",
            "quality_categorization": "Automatically categorize media by quality (high, medium, low, original, thumbnail)",
            "high_quality_optimization": "Auto-optimize images and videos for best quality on Cloudinary"
        },
        "cloudinary_status": "configured" if cloudinary_configured else "not_configured",
        "endpoints": {
